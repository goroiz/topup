...
/* ===== Particles Background (lightweight on mobile) ===== */
(function particles(){
  ...
  function init(){
    ...
    for(let i=0;i<count;i++){
      nodes.push({ x: Math.random()*w, y: Math.random()*h, vx: (Math.random()-.5)*0.18*dpi, vy:(Math.random()-.5)*0.18*dpi, r: Math.random()*2+0.6 });
    }
  }
  function loop(){
...
function pill(el){ el.className = 'pill'; return el; }
function buildDenoms(){
  denomsBox.innerHTML = '';
  DENOMS.forEach(d=>{
    const el = pill(document.createElement('div'));
